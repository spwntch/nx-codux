...
export const Tags: React.FC<ITagsProps> = ({ tags, className }) => {
  return (
    ...
    >
      {tags.map((tag, index) => (
        <div key={index}>
          <Badge className="bg-purple-500 text-white rounded-full px-3 py-1 text-sm whitespace-nowrap overflow-hidden text-ellipsis max-w-[150px] inline-block flex-shrink-0">
            {tag}
          </Badge>
        </div>
      ))}
